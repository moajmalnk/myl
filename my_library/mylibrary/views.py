...
#         next_primary_numbe  = int(last_primary_number.primary_number) + 1
#     else:
...
#             for row in reader:
#                 # Extract data from each row and create Book_details object
#                 primary_number = row['book_no']
#                 name = row['name']
#                 slug = row['slug']
#                 desc = row['desc']
#                 author = row['author']
        ...
        if 'csv' in request.FILES:
            ...
            if csv_file:
                ...
                for row in reader:
                    ...
                    book.save()
                messages.success(request, "Books added successfully from CSV.")
                return redirect('mylibrary:home')
        else:
            primary_number = request.POST.get('primary_number')
            name = request.POST.get('name')
            ...
            if Book_details.objects.filter(primary_number=primary_number).exists():
                messages.error(request, "Book number already exists.")
            elif Book_details.objects.filter(slug=slug).exists():
                messages.error(request, "Book name already exists.")
            else:
                book = Book_details(
                    primary_number=primary_number, name=name,
                    slug=slug, desc=desc, author=author, category=category,
                    price=price, stock=stock, lang=lang
                )
                book.save()
    ...
    if request.method == 'POST':
        ...
        if name:
            ...
            if Category.objects.filter(slug=slug).exists():
                ...
                category = Category(name=name, slug=slug, img=img)
                category.save()
...
def add_language(request):
    if request.method == 'POST':
        ...
        if name:
            if not re.match(r'^[A-Za-z\s]+$', name):
                messages.warning(request, "Language name should only contain alphabetic characters.")
                return redirect('search_app:add_language')
            if Language.objects.filter(slug=slug).exists():
                messages.info(request, 'LANGUAGE NAME EXISTS')
                return redirect('search_app:add_language')
            else:
                language = Language(name=name, slug=slug)
                language.save()
                messages.info(request, f'The {name} Language successfully added')
        else:
            messages.warning(request, "Please enter a valid name.")
...
def all_book_language(request, l_slug=None):
    l_page = None

    if l_slug is not None:
        l_page = get_object_or_404(Language, slug=l_slug)
        books_list = Book_details.objects.filter(lang=l_page, available=True).order_by('id')
    else:
        books_list = Book_details.objects.filter(available=True).order_by('id')
    context = {
        'language': l_page,
        ...
        if amount_paid is not None and amount_paid != '':
            try:
                ...
                if amount_paid >= 0:
                    ...
                    messages.success(request, f"{patron.paid} fine has been paid for {patron.name}.")
                else:
